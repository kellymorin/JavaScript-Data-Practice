...
githubData.forEach((event)=>{
  ...
  if(githubData[i].hasOwnProperty("payload")){
    if(githubData[i].payload.hasOwnProperty("commits")){
      let commitCount = githubData[i].payload.commits.length;
      count += commitCount;
    } else if (githubData[i].payload.hasOwnProperty("pull_request")){
      let prCommitCount = githubData[i].payload.pull_request.commits;
      count += prCommitCount;
    }
  }
})
...
function eventTypeCount (data){
  data.forEach((event)=>{
    let i = githubData.indexOf(event)
    typeArray.push(githubData[i].type)
    // console.log(typeArray);
  }); typeArray.forEach((value)=>{
      typeObject[value] = (typeObject[value] || 0)+1;
  })
} eventTypeCount(githubData);
    ...
    if(eventData === "payload"){
      if(githubData[i].payload.hasOwnProperty("pull_request")){
        if(githubData[i].payload.pull_request.user.login !== "stevebrownlee" && githubData[i].payload.pull_request.merge_commit_sha !== null){
          users.push(githubData[i].payload.pull_request.user.login)
        }
      }
    }
  }
...
function repo (data){
  data.forEach((event)=>{
    let i = githubData.indexOf(event);
    if(githubData[i].hasOwnProperty("repo")){
      if(githubData[i].repo.hasOwnProperty("id")){
        repoList.push(githubData[i].repo.id);
        console.log(repoList)
      }
    }
  }); repoList.forEach((value)=>{
      repoObject[value] = (repoObject[value] || 0)+1;
  })
}
repo(githubData);
  ...
  let eventID = githubData[i].id;
    for(const eventData in event){
      if(eventData === "payload"){
        if(githubData[i].payload.hasOwnProperty("commits")){
          let commitCount = githubData[i].payload.commits.length;
          // console.log(eventID, commitCount);
          commitArray.push(`{eventID: ${eventID}, commitCount: ${commitCount}},`);
          } else if(githubData[i].payload.hasOwnProperty("pull_request")){
            if(githubData[i].payload.pull_request.hasOwnProperty("commits")){
              let prCommitCount = githubData[i].payload.pull_request.commits;
              // console.log(prCommitCount);
              commitArray.push(`{eventID: ${eventID}, commitCount: ${prCommitCount}}`); 
            }
          }
        }
      }
...
function languages (data){
  data.forEach((event)=> {
    ...
    if(event.type === "PullRequestEvent"){
      languageList.push(githubData[i].payload.pull_request.base.repo.language);
      languageList.push(githubData[i].payload.pull_request.head.repo.language);
    } 
  }); languageList.forEach((value)=>{
      languageObject[value] =(languageObject[value] || 0)+1;
  })
} 
